...
export class Board {
    constructor() {
        ...
        this.board = Array.from({ length: Shogi.boardSize }, () => Array(Shogi.boardSize).fill(new Shogi.Air()));
    ...
    move([fromRow, fromCol], [toRow, toCol]) {
        ...
        let canPutPositions = this.board[fromRow][fromCol].generateMovePositions([fromRow, fromCol]);
        ...
        for (let pos = putIndex - 1; pos >= 0; pos--) {
            ...
            if (row == Shogi.partition[0] && col == Shogi.partition[1])
                break;
        ...
        if (this.board[toRow][toCol].isSente !== undefined) {
            if (this.isSenteTurn)
                this.senteHave.push(this.board[toRow][toCol]);
            else
                this.goteHave.push(this.board[toRow][toCol]);
        }
        // Move the piece
