...
export class Board {
     ...
     */
    moveCapturedPiece(capturedIndex, [row, col]) {
        let capturedPieces = this.isSenteTurn ? this.senteCapturedPieces : this.goteCapturedPieces;
        // Out of range.
        ...
        if (capturedPieces[capturedIndex].toString() === "歩") {
            let pawnExistsInColumn = this.matrix.some(row => row[col].isSente === this.isSenteTurn && (row[col].toString() === "歩" || row[col].toString() === "と"));
